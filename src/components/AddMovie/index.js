...
export const AddMovie = () => {
  ...
  const renderFormik = (formik) => {
    return (
      <ModalContainer>
        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.TITLE]}
        </ModalTitle>
        <ModalInput name="title" type="text" />
        <ErrorText name="title" component="div" />

        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.RELEASE_DATE]}
        </ModalTitle>
        <SelectorContainer>
          <DateSelector
            ...
            name="date"
            placeholderText={
              TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.SELECT_DATE]
            }
          />
          <IconBox size={20} />
        </SelectorContainer>
        ...
        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.MOVIE_URL]}
        </ModalTitle>
        <ModalInput name="poster_path" type="text" />
        <ErrorText name="poster_path" component="div" />

        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.GENRE]}
        </ModalTitle>
        <SelectorContainer>
          <DropDown control="select" name="genres" options={dropdownOptions} />
          <ArrowBox size={20} />
        </SelectorContainer>
        <ErrorText name="genre" component="div" />

        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.OVERVIEW]}
        </ModalTitle>
        <ModalInput name="overview" type="text" />
        <ErrorText name="overview" component="div" />

        <ModalTitle>
          {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.RUNTIME]}
        </ModalTitle>
        ...
        <ButtonContainer>
          <ResetButton onClick={onReset}>
            {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.RESET]}
          </ResetButton>
          <SubmitButton type="submit" primary>
            {TRANSLATIONS[I18N_LANGUAGES.EN][I18N_KEYS.SUBMIT]}
          </SubmitButton>
        </ButtonContainer>
      </ModalContainer>
    );
  ...
  return (
    <AddMovieButtonContainer>
      ...
      <AddMovieModal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        <Formik initialValues={initialValues}>{renderFormik}</Formik>
      </AddMovieModal>
    </AddMovieButtonContainer>
  );
};
